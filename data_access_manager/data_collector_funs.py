...
class DataCollector:
    ...
    def get_mongodb_projects_ids(self, collection_name, old_collection=False):
        ...
        :param old_collection: True for old collection format
    ...
    def get_text_mongodb(self, project_id, collection_name):
        """
        get text from MongoDB
    ...
    def insert_predictions(self, project_id, location_information, version='v2',
    ...
    def free_select_sql(self, sql):
        ...
        import csv
        with open('V5_Project_Location_New_coll_DB_tmp.csv', 'w', encoding='utf-8', newline='') as out:
    ...
    def get_project_ids(self, projects_files_path, collection_name):
        mysql_ids = self.get_mysql_projects_ids()

        if projects_files_path is None:
            mongo_ids = self.get_mongodb_projects_ids(collection_name)
...
if __name__ == '__main__':
    ...
    # loc = data_collector.free_select_sql(
    #     "DELETE FROM ProjectLocationNew WHERE CityName ='' and Version = 'v2'  and DataTrace = 'Predicted by Location Algorithm V2.0' ")
